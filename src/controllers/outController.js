...
const getPenggajian = async (req, res) =>{
    ...
    // console.log(filter);
    getmenu(function(listmenu) {
        outGaji.getDataPenggajian(function (dataPenggajian) {
            res.render('./pages/gj-penggajian',{
                ...
                listmenu,
                filter,
                dataGaji: dataPenggajian.row
            })
        })
    })
}

const postPenggajian = async (req, res) => {
    const { kd, id, timestamp, nama, jabatan, gajikotor, ntpotongan, jtpotongan, ntunjangan, jtunjangan, gjbersih, } = req.body;

    // jtpotongan.map(pot => {
    //     const result = pot.replace(/[.]/g, '')
    //     return parseInt(result)
    // })

    // jtunjangan.map(tunj => {
    //     const result = tunj.replace(/[.]/g, '')
    //     return parseInt(result)
    // })

    // console.log('ini Potongan = ', jtpotongan)
    // console.log('ini Tunjangan = ', jtunjangan)

    outGaji.addDataGaji(kd, id, timestamp, nama, jabatan, gajikotor, ntpotongan, jtpotongan, ntunjangan, jtunjangan, gjbersih, function (data) {
        if (data.status === 'created') {
            req.flash('success', 'Data Gaji Berhasil Ditambahkan')
            res.redirect('/gj/penggajian')
        } else {
            req.flash('error', 'ID Sudah ada')
            res.redirect('/gj/penggajian')
        }
